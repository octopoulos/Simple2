...
void showExampleDialog(entry::AppI* _app, const char* _errorText)
{
	...
	ImGui::TextWrapped("%s", _app ? _app->getDescription() : "DESCRIPTION");

	bx::StringView url = _app ? _app->getUrl() : "URL";
	if (!url.isEmpty())
	{
		ImGui::SameLine();
...
#if 0
	{
		...
		if (0 != num)
		{
			...
			for (uint8_t ii = 0; ii < num; ++ii)
			{
				...
				if (caps->vendorId == gpu.vendorId && caps->deviceId == gpu.deviceId)
					current = ii;
			}

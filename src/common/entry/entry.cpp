	...
	do
	{
		...
		if (ev)
		{
			...
			case Event::Mouse:
			{
				...
				GetGlobalInput().MouseMove(mouse->device, mouse->finger, mouse->m_mx, mouse->m_my, mouse->m_mz, mouse->hasDelta, mouse->m_dx, mouse->m_dy, mouse->pressure);
				if (!mouse->m_move)
		...
		if (ev)
		{
			...
			case Event::Mouse:
			{
				...
				if (!mouseLock)
				{
					...
					else win.m_mouse.m_buttons[mouse->m_button] = mouse->m_down;
				}
			}
			break;

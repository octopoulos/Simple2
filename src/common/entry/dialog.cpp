...
#if BX_PLATFORM_WINDOWS
	void* result = ShellExecuteA(nullptr, nullptr, tmp, nullptr, nullptr, false);
	BX_UNUSED(result);
...
public:
	...
	bx::StringView next()
	{
		...
		m_token = bx::strTrim(
		    bx::StringView(
		        m_token.getTerm() + 1, bx::strFind(bx::StringView(m_token.getTerm() + 1, m_str.getTerm()), m_ch).getPtr()),
		    " \t\n");
...
static const struct
{
	OPENFILENAMEFUNCTION m_function;
	uint32_t             m_flags;
} s_getFileNameA[] = {
...
bool openFileSelectionDialog(
    bx::FilePath& _inOutFilePath, FileSelectionDialogType::Enum _type, const bx::StringView& _title, const bx::StringView& _filter)
{
	bx::Error err;
...
#	if BX_PLATFORM_LINUX
	bx::write(&writer, &err, "--file-selection%s --title \"%.*s\" --filename \"%s\"", FileSelectionDialogType::Save == _type ? " --save" : "", _title.getLength(), _title.getPtr(), _inOutFilePath.getCPtr());

	for (bx::LineReader lr(_filter); !lr.isDone();)
	{
		const bx::StringView line = lr.next();

		bx::write(&writer, &err, " --file-filter \"%.*s\"", line.getLength(), line.getPtr());
	}

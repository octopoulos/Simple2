...
// Needs some bits to get jiggled to work with other players.
...
struct AviWriter
{
	...
	{
		if (!bx::open(m_writer, _filePath)) return false;
		...
		// Bgfx returns _yflip true for OpenGL since bottom left corner is 0, 0.
		// In D3D top left corner is 0, 0. DIB expect OpenGL style coordinates, so this is inverted logic for AVI writer.
		m_yflip = !_yflip;

...
struct Context
{
	    ...
	    , m_exit(false)
	{
		...
		// WinUser.h
		TRANSLATE_KEY(uint8_t('A'), Key::KeyA);
		TRANSLATE_KEY(uint8_t('B'), Key::KeyB);
		TRANSLATE_KEY(uint8_t('C'), Key::KeyC);
		TRANSLATE_KEY(uint8_t('D'), Key::KeyD);
		TRANSLATE_KEY(uint8_t('E'), Key::KeyE);
		TRANSLATE_KEY(uint8_t('F'), Key::KeyF);
		TRANSLATE_KEY(uint8_t('G'), Key::KeyG);
		TRANSLATE_KEY(uint8_t('H'), Key::KeyH);
		TRANSLATE_KEY(uint8_t('I'), Key::KeyI);
		TRANSLATE_KEY(uint8_t('J'), Key::KeyJ);
		TRANSLATE_KEY(uint8_t('K'), Key::KeyK);
		TRANSLATE_KEY(uint8_t('L'), Key::KeyL);
		TRANSLATE_KEY(uint8_t('M'), Key::KeyM);
		TRANSLATE_KEY(uint8_t('N'), Key::KeyN);
		TRANSLATE_KEY(uint8_t('O'), Key::KeyO);
		TRANSLATE_KEY(uint8_t('P'), Key::KeyP);
		TRANSLATE_KEY(uint8_t('Q'), Key::KeyQ);
		TRANSLATE_KEY(uint8_t('R'), Key::KeyR);
		TRANSLATE_KEY(uint8_t('S'), Key::KeyS);
		TRANSLATE_KEY(uint8_t('T'), Key::KeyT);
		TRANSLATE_KEY(uint8_t('U'), Key::KeyU);
		TRANSLATE_KEY(uint8_t('V'), Key::KeyV);
		TRANSLATE_KEY(uint8_t('W'), Key::KeyW);
		TRANSLATE_KEY(uint8_t('X'), Key::KeyX);
		TRANSLATE_KEY(uint8_t('Y'), Key::KeyY);
		TRANSLATE_KEY(uint8_t('Z'), Key::KeyZ);

		TRANSLATE_KEY(uint8_t('1'), Key::Key1);
		TRANSLATE_KEY(uint8_t('2'), Key::Key2);
		TRANSLATE_KEY(uint8_t('3'), Key::Key3);
		TRANSLATE_KEY(uint8_t('4'), Key::Key4);
		TRANSLATE_KEY(uint8_t('5'), Key::Key5);
		TRANSLATE_KEY(uint8_t('6'), Key::Key6);
		TRANSLATE_KEY(uint8_t('7'), Key::Key7);
		TRANSLATE_KEY(uint8_t('8'), Key::Key8);
		TRANSLATE_KEY(uint8_t('9'), Key::Key9);
		TRANSLATE_KEY(uint8_t('0'), Key::Key0);

		TRANSLATE_KEY(VK_RETURN, Key::Return);
		TRANSLATE_KEY(VK_ESCAPE, Key::Esc);
		TRANSLATE_KEY(VK_BACK, Key::Backspace);
		TRANSLATE_KEY(VK_TAB, Key::Tab);
		TRANSLATE_KEY(VK_SPACE, Key::Space);

		TRANSLATE_KEY(VK_OEM_MINUS, Key::Minus);
		TRANSLATE_KEY(VK_OEM_PLUS, Key::Equals);
		TRANSLATE_KEY(VK_OEM_4, Key::LeftBracket);
		TRANSLATE_KEY(VK_OEM_6, Key::RightBracket);
		TRANSLATE_KEY(VK_OEM_5, Key::Backslash);
		TRANSLATE_KEY(VK_OEM_1, Key::Semicolon);
		TRANSLATE_KEY(VK_OEM_7, Key::Quote);
		TRANSLATE_KEY(VK_OEM_3, Key::Tilde);
		TRANSLATE_KEY(VK_OEM_COMMA, Key::Comma);
		TRANSLATE_KEY(VK_OEM_PERIOD, Key::Period);
		TRANSLATE_KEY(VK_OEM_2, Key::Slash);

		TRANSLATE_KEY(VK_CAPITAL, Key::CapsLock);

		TRANSLATE_KEY(VK_F1, Key::F1);
		TRANSLATE_KEY(VK_F2, Key::F2);
		TRANSLATE_KEY(VK_F3, Key::F3);
		TRANSLATE_KEY(VK_F4, Key::F4);
		TRANSLATE_KEY(VK_F5, Key::F5);
		TRANSLATE_KEY(VK_F6, Key::F6);
		TRANSLATE_KEY(VK_F7, Key::F7);
		TRANSLATE_KEY(VK_F8, Key::F8);
		TRANSLATE_KEY(VK_F9, Key::F9);
		TRANSLATE_KEY(VK_F10, Key::F10);
		TRANSLATE_KEY(VK_F11, Key::F11);
		TRANSLATE_KEY(VK_F12, Key::F12);

		TRANSLATE_KEY(VK_SNAPSHOT, Key::Print);
		TRANSLATE_KEY(VK_SCROLL, Key::ScrollLock);
		TRANSLATE_KEY(VK_PAUSE, Key::Pause);
		TRANSLATE_KEY(VK_INSERT, Key::Insert);
		TRANSLATE_KEY(VK_HOME, Key::Home);
		TRANSLATE_KEY(VK_PRIOR, Key::PageUp);
		TRANSLATE_KEY(VK_DELETE, Key::Delete);
		TRANSLATE_KEY(VK_END, Key::End);
		TRANSLATE_KEY(VK_NEXT, Key::PageDown);
		TRANSLATE_KEY(VK_RIGHT, Key::Right);
		TRANSLATE_KEY(VK_LEFT, Key::Left);
		TRANSLATE_KEY(VK_DOWN, Key::Down);
		TRANSLATE_KEY(VK_UP, Key::Up);

		TRANSLATE_KEY(VK_NUMLOCK, Key::NumLockClear);
		TRANSLATE_KEY(VK_DIVIDE, Key::NumPadDivide);
		TRANSLATE_KEY(VK_MULTIPLY, Key::NumPadMultiply);
		TRANSLATE_KEY(VK_SUBTRACT, Key::NumPadMinus);
		TRANSLATE_KEY(VK_ADD, Key::NumPadPlus);
		TRANSLATE_KEY(VK_SEPARATOR, Key::NumPadEnter);
		TRANSLATE_KEY(VK_NUMPAD1, Key::NumPad1);
		TRANSLATE_KEY(VK_NUMPAD2, Key::NumPad2);
		TRANSLATE_KEY(VK_NUMPAD3, Key::NumPad3);
		TRANSLATE_KEY(VK_NUMPAD4, Key::NumPad4);
		TRANSLATE_KEY(VK_NUMPAD5, Key::NumPad5);
		TRANSLATE_KEY(VK_NUMPAD6, Key::NumPad6);
		TRANSLATE_KEY(VK_NUMPAD7, Key::NumPad7);
		TRANSLATE_KEY(VK_NUMPAD8, Key::NumPad8);
		TRANSLATE_KEY(VK_NUMPAD9, Key::NumPad9);
		TRANSLATE_KEY(VK_NUMPAD0, Key::NumPad0);
		TRANSLATE_KEY(VK_DECIMAL, Key::Period);

		TRANSLATE_KEY(VK_OEM_102, Key::NonUsBackslash);
		TRANSLATE_KEY(VK_APPS, Key::Application);
		TRANSLATE_KEY(VK_VOLUME_MUTE, Key::Mute);
		TRANSLATE_KEY(VK_VOLUME_UP, Key::VolumeUp);
		TRANSLATE_KEY(VK_VOLUME_DOWN, Key::VolumeDown);

		TRANSLATE_KEY(VK_CLEAR, Key::Clear);

		TRANSLATE_KEY(VK_LCONTROL, Key::LeftCtrl);
		TRANSLATE_KEY(VK_CONTROL, Key::LeftCtrl);
		TRANSLATE_KEY(VK_LSHIFT, Key::LeftShift);
		TRANSLATE_KEY(VK_SHIFT, Key::LeftShift);
		TRANSLATE_KEY(VK_LMENU, Key::LeftAlt);
		TRANSLATE_KEY(VK_MENU, Key::LeftAlt);
		TRANSLATE_KEY(VK_LWIN, Key::LeftMeta);
		TRANSLATE_KEY(VK_RCONTROL, Key::RightCtrl);
		TRANSLATE_KEY(VK_RSHIFT, Key::RightShift);
		TRANSLATE_KEY(VK_RMENU, Key::RightAlt);
		TRANSLATE_KEY(VK_RWIN, Key::RightMeta);

		TRANSLATE_KEY(VK_MEDIA_NEXT_TRACK, Key::MediaNext);
		TRANSLATE_KEY(VK_MEDIA_PREV_TRACK, Key::MediaPrevious);
		TRANSLATE_KEY(VK_MEDIA_STOP, Key::MediaStop);
		TRANSLATE_KEY(VK_MEDIA_PLAY_PAUSE, Key::MediaPlayPause);
		TRANSLATE_KEY(VK_LAUNCH_MEDIA_SELECT, Key::MediaSelect);

		TRANSLATE_KEY(VK_LAUNCH_APP1, Key::App1);
		TRANSLATE_KEY(VK_LAUNCH_APP2, Key::App2);
	}
	...
	LRESULT process(HWND _hwnd, UINT _id, WPARAM _wparam, LPARAM _lparam)
	{
		if (m_init)
		{
			...
			{
				Msg* msg = (Msg*)_lparam;
				SetWindowPos(m_hwnd[_wparam], 0, msg->m_x, msg->m_y, 0, 0, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOSIZE);
				delete msg;
			}
			break;
	...
	void adjust(HWND _hwnd, uint32_t _width, uint32_t _height, bool _windowFrame)
	{
		...
		SetWindowPos(_hwnd, HWND_TOP, left, top, width, height, SWP_SHOWWINDOW);

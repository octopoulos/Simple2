...
struct GamepadGLFW
{
	...
	void update(EventQueue& _eventQueue)
	{
		...
		for (int ii = 0; ii < numAxes; ++ii)
		{
			...
			{
				m_axes[ii] = value;
				_eventQueue.postAxisEvent(defaultWindow, m_handle, axis, value);
			}
		}
		...
		for (int ii = 0; ii < numButtons; ++ii)
		{
			...
			if (m_buttons[ii] != buttons[ii])
			{
				...
				_eventQueue.postKeyEvent(defaultWindow, key, 0, buttons[ii] != 0);
			}
		}
	}
	...
	int run(int _argc, const char* const* _argv)
	{
		...
		for (uint32_t ii = 0; ii < ENTRY_CONFIG_MAX_GAMEPADS; ++ii)
		{
			...
			if (glfwJoystickPresent(ii))
			{
				...
				m_eventQueue.postGamepadEvent(handle, m_gamepad[ii].m_handle, true);
			}
		}

		m_thread.init(MainThreadEntry::threadFunc, &m_mte);

		while (NULL != m_window[0] && !glfwWindowShouldClose(m_window[0]))
		{
			...
			while (Msg* msg = m_msgs.pop())
			{
				...
				case GLFW_WINDOW_TOGGLE_FULL_SCREEN:
				{
					...
					if (glfwGetWindowMonitor(window))
						glfwSetWindowMonitor(window, NULL, m_oldX, m_oldY, m_oldWidth, m_oldHeight, 0);
					else
					{

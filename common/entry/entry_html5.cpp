...
struct Context
{
	    ...
	    , m_scroll(0)
	{
		...
		TRANSLATE_U8('\n', Key::Return);
		TRANSLATE_U8('\r', Key::Return);
		TRANSLATE_KEY(27, Key::Esc);
		TRANSLATE_KEY(127, Key::Backspace);
		TRANSLATE_U8('\t', Key::Tab);
		TRANSLATE_U8(' ', Key::Space);

		TRANSLATE_U8('_', Key::Minus);
		TRANSLATE_U8('-', Key::Minus);
		TRANSLATE_U8('+', Key::Equals);
		TRANSLATE_U8('=', Key::Equals);
		TRANSLATE_U8('{', Key::LeftBracket);
		TRANSLATE_U8('[', Key::LeftBracket);
		TRANSLATE_U8('}', Key::RightBracket);
		TRANSLATE_U8(']', Key::RightBracket);
		TRANSLATE_U8('|', Key::Backslash);
		TRANSLATE_U8('\\', Key::Backslash);
		TRANSLATE_U8(':', Key::Semicolon);
		TRANSLATE_U8(';', Key::Semicolon);
		TRANSLATE_U8('"', Key::Quote);
		TRANSLATE_U8('\'', Key::Quote);
		TRANSLATE_U8('~', Key::Tilde);
		TRANSLATE_U8('`', Key::Tilde);
		TRANSLATE_U8('<', Key::Comma);
		TRANSLATE_U8(',', Key::Comma);
		TRANSLATE_U8('>', Key::Period);
		TRANSLATE_U8('.', Key::Period);
		TRANSLATE_U8('?', Key::Slash);
		TRANSLATE_U8('/', Key::Slash);

		TRANSLATE_KEY(39, Key::Right);
		TRANSLATE_KEY(37, Key::Left);
		TRANSLATE_KEY(40, Key::Down);
		TRANSLATE_KEY(38, Key::Up);
	}
	...
	if (_event)
	{
		...
		case EMSCRIPTEN_EVENT_DBLCLICK:
			...
			s_ctx.m_eventQueue.postMouseEvent(
			    kDefaultWindowHandle, s_ctx.m_mx, s_ctx.m_my, s_ctx.m_scroll, mb, (_eventType != EMSCRIPTEN_EVENT_MOUSEUP));
			return true;
		}
	}
...
uint8_t translateModifiers(const EmscriptenKeyboardEvent* _event)
{
	...
	if (_event->shiftKey) mask |= Modifier::LeftShift | Modifier::RightShift;

	if (_event->altKey)   mask |= Modifier::LeftAlt | Modifier::RightAlt;

	if (_event->ctrlKey)  mask |= Modifier::LeftCtrl | Modifier::RightCtrl;

	if (_event->metaKey)  mask |= Modifier::LeftMeta | Modifier::RightMeta;
	// clang-format on
	return mask;
}

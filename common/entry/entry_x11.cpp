...
struct Joystick
{
	...
	{
		if (-1 == m_fd) return false;

		JoystickEvent event;
		int32_t       bytes = read(m_fd, &event, sizeof(JoystickEvent));
		if (bytes != sizeof(JoystickEvent)) return false;
			...
			if (event.number < BX_COUNTOF(s_translateAxis))
			{
				...
				if (filter(axis, &value))
				{
					...
					if (Key::None != s_axisDpad[axis].first)
					{
						...
						{
							_eventQueue.postKeyEvent(defaultWindow, 0 > m_value[axis] ? s_axisDpad[axis].first : s_axisDpad[axis].second, 0, true);
						}
					}
				}
			}
...
static void TRANSLATE_KEY(uint16_t _xk, Key::Enum _key)
{
	_xk += 256;
...
struct Context
{
	    ...
	    , m_exit(false)
	{
		...
		// https://doc.servo.org/src/x11/keysym.rs.html
		TRANSLATE_U8('a', Key::KeyA);
		TRANSLATE_U8('b', Key::KeyB);
		TRANSLATE_U8('c', Key::KeyC);
		TRANSLATE_U8('d', Key::KeyD);
		TRANSLATE_U8('e', Key::KeyE);
		TRANSLATE_U8('f', Key::KeyF);
		TRANSLATE_U8('g', Key::KeyG);
		TRANSLATE_U8('h', Key::KeyH);
		TRANSLATE_U8('i', Key::KeyI);
		TRANSLATE_U8('j', Key::KeyJ);
		TRANSLATE_U8('k', Key::KeyK);
		TRANSLATE_U8('l', Key::KeyL);
		TRANSLATE_U8('m', Key::KeyM);
		TRANSLATE_U8('n', Key::KeyN);
		TRANSLATE_U8('o', Key::KeyO);
		TRANSLATE_U8('p', Key::KeyP);
		TRANSLATE_U8('q', Key::KeyQ);
		TRANSLATE_U8('r', Key::KeyR);
		TRANSLATE_U8('s', Key::KeyS);
		TRANSLATE_U8('t', Key::KeyT);
		TRANSLATE_U8('u', Key::KeyU);
		TRANSLATE_U8('v', Key::KeyV);
		TRANSLATE_U8('w', Key::KeyW);
		TRANSLATE_U8('x', Key::KeyX);
		TRANSLATE_U8('y', Key::KeyY);
		TRANSLATE_U8('z', Key::KeyZ);

		TRANSLATE_U8('0', Key::Key0);
		TRANSLATE_U8('1', Key::Key1);
		TRANSLATE_U8('2', Key::Key2);
		TRANSLATE_U8('3', Key::Key3);
		TRANSLATE_U8('4', Key::Key4);
		TRANSLATE_U8('5', Key::Key5);
		TRANSLATE_U8('6', Key::Key6);
		TRANSLATE_U8('7', Key::Key7);
		TRANSLATE_U8('8', Key::Key8);
		TRANSLATE_U8('9', Key::Key9);

		TRANSLATE_KEY(XK_Return, Key::Return);
		TRANSLATE_KEY(XK_Escape, Key::Esc);
		TRANSLATE_KEY(XK_BackSpace, Key::Backspace);
		TRANSLATE_KEY(XK_Tab, Key::Tab);
		TRANSLATE_KEY(XK_space, Key::Space);

		TRANSLATE_KEY(XK_minus, Key::Minus);
		TRANSLATE_KEY(XK_equal, Key::Equals);
		TRANSLATE_KEY(XK_bracketleft, Key::LeftBracket);
		TRANSLATE_KEY(XK_bracketright, Key::RightBracket);
		TRANSLATE_KEY(XK_backslash, Key::Backslash);
		TRANSLATE_KEY(XK_semicolon, Key::Semicolon);
		TRANSLATE_KEY(XK_apostrophe, Key::Quote);
		TRANSLATE_KEY(XK_grave, Key::Tilde);
		TRANSLATE_KEY(XK_comma, Key::Comma);
		TRANSLATE_KEY(XK_period, Key::Period);
		TRANSLATE_KEY(XK_slash, Key::Slash);

		TRANSLATE_KEY(XK_Caps_Lock, Key::CapsLock);
		TRANSLATE_KEY(XK_Shift_Lock, Key::CapsLock);

		TRANSLATE_KEY(XK_F1, Key::F1);
		TRANSLATE_KEY(XK_F2, Key::F2);
		TRANSLATE_KEY(XK_F3, Key::F3);
		TRANSLATE_KEY(XK_F4, Key::F4);
		TRANSLATE_KEY(XK_F5, Key::F5);
		TRANSLATE_KEY(XK_F6, Key::F6);
		TRANSLATE_KEY(XK_F7, Key::F7);
		TRANSLATE_KEY(XK_F8, Key::F8);
		TRANSLATE_KEY(XK_F9, Key::F9);
		TRANSLATE_KEY(XK_F10, Key::F10);
		TRANSLATE_KEY(XK_F11, Key::F11);
		TRANSLATE_KEY(XK_F12, Key::F12);

		TRANSLATE_KEY(XK_Print, Key::Print);
		TRANSLATE_KEY(XK_Scroll_Lock, Key::ScrollLock);
		TRANSLATE_KEY(XK_Pause, Key::Pause);
		TRANSLATE_KEY(XK_Insert, Key::Insert);
		TRANSLATE_KEY(XK_Home, Key::Home);
		TRANSLATE_KEY(XK_Page_Up, Key::PageUp);
		TRANSLATE_KEY(XK_Delete, Key::Delete);
		TRANSLATE_KEY(XK_End, Key::End);
		TRANSLATE_KEY(XK_Page_Down, Key::PageDown);
		TRANSLATE_KEY(XK_Right, Key::Right);
		TRANSLATE_KEY(XK_Left, Key::Left);
		TRANSLATE_KEY(XK_Down, Key::Down);
		TRANSLATE_KEY(XK_Up, Key::Up);

		TRANSLATE_KEY(XK_Num_Lock, Key::NumLockClear);
		TRANSLATE_KEY(XK_KP_Divide, Key::NumPadDivide);
		TRANSLATE_KEY(XK_KP_Multiply, Key::NumPadMultiply);
		TRANSLATE_KEY(XK_KP_Subtract, Key::NumPadMinus);
		TRANSLATE_KEY(XK_KP_Add, Key::NumPadPlus);
		TRANSLATE_KEY(XK_KP_End, Key::NumPad1);
		TRANSLATE_KEY(XK_KP_Down, Key::NumPad2);
		TRANSLATE_KEY(XK_KP_Page_Down, Key::NumPad3);
		TRANSLATE_KEY(XK_KP_Left, Key::NumPad4);
		TRANSLATE_KEY(XK_KP_Begin, Key::NumPad5);
		TRANSLATE_KEY(XK_KP_Right, Key::NumPad6);
		TRANSLATE_KEY(XK_KP_Home, Key::NumPad7);
		TRANSLATE_KEY(XK_KP_Up, Key::NumPad8);
		TRANSLATE_KEY(XK_KP_Page_Up, Key::NumPad9);
		TRANSLATE_KEY(XK_KP_Insert, Key::NumPad0);

		TRANSLATE_KEY(XK_KP_Separator, Key::NumPadComma);

		TRANSLATE_KEY(XK_Control_L, Key::LeftCtrl);
		TRANSLATE_KEY(XK_Shift_L, Key::LeftShift);
		TRANSLATE_KEY(XK_Alt_L, Key::LeftAlt);
		TRANSLATE_KEY(XK_Meta_L, Key::LeftMeta);
		TRANSLATE_KEY(XK_Control_R, Key::RightCtrl);
		TRANSLATE_KEY(XK_Shift_R, Key::RightShift);
		TRANSLATE_KEY(XK_Alt_R, Key::RightAlt);
		TRANSLATE_KEY(XK_Meta_R, Key::RightMeta);
	...
	int32_t run(int _argc, const char* const* _argv)
	{
		...
		m_window[0] = XCreateWindow(
		    m_display, m_root, 0, 0, 1, 1, 0, m_depth, InputOutput, m_visual, CWBorderPixel | CWEventMask | CWBackPixel | CWBitGravity, &m_windowAttrs);
		...
		XIC ic;
		ic = XCreateIC(
		    im, XNInputStyle, 0 | XIMPreeditNothing | XIMStatusNothing, XNClientWindow, m_window[0], NULL);
			...
			else
			{
				...
				case ButtonRelease:
				{
					...
					if (MouseButton::None != mb)
						m_eventQueue.postMouseEvent(handle, xbutton.x, xbutton.y, m_mz, mb, event.type == ButtonPress);
					else
	...
	void createWindow(WindowHandle _handle, Msg* msg)
	{
		Window window = XCreateWindow(
		    m_display, m_root, msg->m_x, msg->m_y, msg->m_width, msg->m_height, 0, m_depth, InputOutput, m_visual, CWBorderPixel | CWEventMask | CWBackPixel | CWBitGravity, &m_windowAttrs);
